...
A comprehensive real-time Sign Language translation system that enables bidirectional communication between sign language and English. This project uses advanced machine learning, computer vision, and natural language processing to bridge communication gaps.
...
This system provides a complete solution for sign communication with three main features:

1. **Receptive Mode** - Real-time sign language fingerspelling recognition using webcam
2. **Expressive Mode** - Text-to-sign conversion with 3D avatar animation
3. **Translation Mode** - Bidirectional communication combining both modes

The system uses a combination of MediaPipe for hand landmark detection, TensorFlow/Keras for classification, and LLM integration for Sign Gloss grammar conversion.
...
- **Sign Gloss Conversion**: LLM-powered conversion of English to sign language grammar structure
...
- **Storage**: Full sign language animations stored as landmark coordinate sequences
...
- **Supported Providers**:
  ...
  - OpenAI (fallback)
- **Sign Gloss Conversion**: Uses GPT-4o to convert English to proper sign language grammar
...
└── src/
    ...
    │   └── src/
    │       ├── app/
    │       │   ├── page.tsx           # 🏠 Landing page (animated UI, server status, navigation)
    │       │   ├── recognize/
    │       │   │   └── page.tsx       # 🎥 Fingerspell & Avatar (camera recognition + 3D signing)
    │       │   ├── express/
    │       │   │   └── page.tsx       # 🤖 Express mode (text → Sign Language avatar)
    ...
    │       │   └── useServerStatus.ts # 🔌 React hook — live server connection status
    │       └── ui/                    # UI components
    │
    └── server/                        # Flask backend server
        ...
        └── utils/                     # Core utilities
            ...
            ├── llm.py                 # LLM integration (Sign Gloss)
...
- **LLM API Key** (Optional): Azure OpenAI or OpenAI for Sign Gloss conversion
...
5. **Start the servers**
   ...
   **Option A — One-click (recommended on Windows)**:
   ...
   .\start.ps1
   ```
   This opens **two separate terminal windows** — one for the backend (Flask) and one for the frontend (Next.js). It automatically polls port 1234 and waits for the backend to finish loading TensorFlow + MediaPipe models (~30-60 s) before launching the frontend.
   ...
   **Option B — Manual (two terminals)**:
   ...
   cd src\server
   python server.py
   # Wait for: "* Serving Flask app 'server'" before starting the frontend
   ```
   ```powershell
   ...
   > **Note**: The backend takes **30-60 seconds** to start — TensorFlow, Keras, and MediaPipe all load at startup. Wait for `* Serving Flask app 'server'` before opening the browser.

6. **Access the application**
   - **Landing Page**: http://localhost:3000 — animated home with server status badge, feature cards, and navigation
   - **Fingerspell & Avatar** *(recommended)*: http://localhost:3000/recognize — webcam recognition + 3D avatar side-by-side
   - **Express Mode**: http://localhost:3000/express — type text → 3D signing avatar
...
When the frontend loads, you are greeted by an interactive landing page (`/`) that:
- Displays animated sign language alphabet letters floating in the background
- Shows a live **server connection badge** (green = connected, yellow = connecting, red = offline)
- Lists key stats: 26 letters, 21 hand landmarks, 3D avatar renderer, AI-powered LLM
- Provides clickable **feature cards** to navigate to each mode

Click **Fingerspell & Avatar** to open the main combined interface at `/recognize`.

---

### Fingerspell & Avatar Mode (combined)

1. Navigate to http://localhost:3000/recognize (or click the card on the landing page)
...
**Typical Hand Landmarks (21 points)**

- Here’s how they’re usually defined:

- Wrist (1 point)

- Thumb (4 points): tip, joint positions
...
### Express Mode (English → Sign Language)
...
**System Behavior**:
  - Converts English to Sign Gloss grammar automatically
...
The project includes several scripts for collecting and processing sign language data:
...
```
Downloads sign language videos from online sources for database population.
...
2. Convert to Sign Gloss using LLM:
...
7. **Sign Gloss**: LLM conversion may not be grammatically perfect
...
- [ ] Multi-user video chat with sign language overlay
- [ ] Sign language learning mode
- [ ] Export transcription to text file
- [ ] Integration with video conferencing platforms

### Research Directions
- [ ] Improve Sign Gloss conversion accuracy
...
1. **Data Collection**: Record more sign language videos
...
- **OpenAI/Azure**: GPT models for Sign Gloss
- **pgvector**: Vector similarity search
- **Sign Language Community**: For sign language resources and data
...
- [Sign University](https://www.lifeprint.com/)
- [HandSpeak Sign Dictionary](https://www.handspeak.com/)
